...
def ObtenerRestaurantes(request):
    lista = list(restaurants.objects())
    
    for i in range(0,len(lista)):
        lista[i] = lista[i].name
...
def Map(request):
    if request.method == 'POST':
        ...
        return render(request, './restaurante/mapa.html', context)
    else:
        return render(request, './restaurante/mapa.html')
...
def Graph(request):
    ...
    return render(request, './restaurante/grafica.html', context)
